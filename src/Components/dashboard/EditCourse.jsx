...
function EditCourse() {
   ...
   try {
    ...
    if (docSnap.exists()) {
     ...
     if (data.en && data.ar && data.en.Options?.length > 0 && data.ar.Options?.length > 0) {
      const option = data.en.Options[0];
      ...
      const ttNo = selectedLevel.totalTime.split(" ");
      const ttNoAr = selectedLevelAr.totalTime.split(" ");
 ...
 const handleUpdate = async (e) => {
  e.preventDefault();
  if (!priceBefore || !priceAfter || !sessionNo || !totalTime || !sessionPerWeek || !scheduleType || !levelno) {
  ...
  try {
   ...
   const updatedEnOptions = courseData.en.Options.map(option => option.levelno === levelno
    ? { ...option, priceBefore, priceAfter, sessionNo, totalTime, sessionPerWeek, hours, scheduleType, levelno } : option);

   const updatedArOptions = courseData.ar.Options.map(option =>
    option.levelno === levelnoAr
     ? { ...option, priceBefore, priceAfter, sessionNo, totalTime, sessionPerWeek, hours, scheduleType: scheduleTypeAr, levelno: levelnoAr } : option);
   ...
   <form onSubmit={handleUpdate} className='space-y-10'>
    ...
    <section className='grid grid-cols-2 gap-x-8 space-y-6'>
     ...
     <article className='space-y-2'>
      ...
      <input value={hours} type='number' onChange={e => setSessionNo(e.target.value)} className='w-full bg-[var(--Input)] focus:outline-0 py-2 px-4 rounded-lg' />
     </article>
    ...
    <div className="flex justify-between items-center text-xl">
     ...
     <button className="px-20 py-4 shadow-md rounded-xl bg-[var(--Yellow)]">
      Save
     </button>
    </div>
   </form>
